const localStoragePrefix = 'seats8_';
const seats = document.getElementById('seats8');
const selectedSeats8 = [];
...
for (let i = 1; i <= 78; i++) {
  ...
  seat.addEventListener('click', () => {
    if (selectedSeats8.includes(i)) {
      // Deselect seat
      selectedSeats8.splice(selectedSeats8.indexOf(i), 1);
      seat.classList.remove('selected');
    } else {
      // Select seat
      selectedSeats8.push(i);
...
confirmBtn.addEventListener('click', () => {
  selectedSeats8.forEach((seatNum) => {

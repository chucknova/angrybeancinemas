const localStoragePrefix = 'seats9_';
const seats = document.getElementById('seats9');
const selectedSeats9 = [];
...
for (let i = 1; i <= 78; i++) {
  ...
  seat.addEventListener('click', () => {
    if (selectedSeats9.includes(i)) {
      // Deselect seat
      selectedSeats9.splice(selectedSeats9.indexOf(i), 1);
      seat.classList.remove('selected');
    } else {
      // Select seat
      selectedSeats9.push(i);
...
confirmBtn.addEventListener('click', () => {
  selectedSeats9.forEach((seatNum) => {

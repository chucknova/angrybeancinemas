const localStoragePrefix = 'seats5_';
const seats = document.getElementById('seats5');
const selectedSeats5 = [];
...
for (let i = 1; i <= 78; i++) {
  ...
  seat.addEventListener('click', () => {
    if (selectedSeats5.includes(i)) {
      // Deselect seat
      selectedSeats5.splice(selectedSeats5.indexOf(i), 1);
      seat.classList.remove('selected');
    } else {
      // Select seat
      selectedSeats5.push(i);
...
confirmBtn.addEventListener('click', () => {
  selectedSeats5.forEach((seatNum) => {

const localStoragePrefix = 'seats4_';
const seats = document.getElementById('seats4');
const selectedSeats4 = [];
...
for (let i = 1; i <= 78; i++) {
  ...
  seat.addEventListener('click', () => {
    if (selectedSeats4.includes(i)) {
      // Deselect seat
      selectedSeats4.splice(selectedSeats4.indexOf(i), 1);
      seat.classList.remove('selected');
    } else {
      // Select seat
      selectedSeats4.push(i);
...
confirmBtn.addEventListener('click', () => {
  selectedSeats4.forEach((seatNum) => {

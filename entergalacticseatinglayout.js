const localStoragePrefix = 'seats7_';
const seats = document.getElementById('seats7');
const selectedSeats7 = [];
...
for (let i = 1; i <= 78; i++) {
  ...
  seat.addEventListener('click', () => {
    if (selectedSeats7.includes(i)) {
      // Deselect seat
      selectedSeats7.splice(selectedSeats7.indexOf(i), 1);
      seat.classList.remove('selected');
    } else {
      // Select seat
      selectedSeats7.push(i);
...
confirmBtn.addEventListener('click', () => {
  selectedSeats7.forEach((seatNum) => {
